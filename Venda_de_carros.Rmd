...
```{r}
temporal<-ggplot(vanuais, aes(Ano, Total,colour= Escala))+
  geom_line(size= 1)+
  geom_point(size= 4)+
...
```{r}
custo<-ggplot(entrega, aes(Fabricante, valor_entrega))+
  geom_col(fill="black")+
...
```{r}
maodeobra<-ggplot(mobra, aes(Estado, Media_MaoDeObra))+
  geom_col(fill="black")+
  coord_flip()+
...
```{r}
lum<-ggplot(lucro, aes(Fabricante, lucro))+
  geom_col(fill= "black")+
  coord_flip()+
  ylab("Lucro")+
  xlab("Fabricante")+
